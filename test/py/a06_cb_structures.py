...
from node import TYPE_FIELDS, TREE_CHAIN
from node import DECL_NAME, TYPE_NAME
from gcc.tree_code_base import BOOLEAN_TYPE, INTEGER_TYPE, REAL_TYPE, RECORD_TYPE, VAR_DECL
from print_tree import node_as_string


def structure_type_dump(t):
    print(
        "\n# {} {}:{}".format
        (
            DECL_SOURCE_FILE(t),
    ...
    if tc == BOOLEAN_TYPE:
        ...
        # print("{} {}".format(id, node_as_string(TREE_TYPE(t))))
        tf = TYPE_FIELDS (TREE_TYPE(t))
        while not IS_NULL_TREE (tf):
            print(node_as_string(tf, 'fields:'))
            print ("field_name: {}".format(IDENTIFIER_POINTER(DECL_NAME(tf))))
            tf = TREE_CHAIN (tf)
    else:
        print(
